...
class OpenOCDSTM32Service {
    ...
    async detectSTLink() {
        ...
        // Note: This is only for Micro Edge, droplet always uses OpenOCD path to ensure device is actually detected
        if (this.currentDeviceType !== 'DROPLET') {
            try {
                const probes = await this.listCubeProbes();
                if (probes && probes.found) {
                    return {
        ...
        try {
            ...
            for (const speed of speeds) {
                ...
                for (const resetConfig of resetConfigs) {
                    ...
                    // Check flash protection status only if device was actually detected
                    let protectionStatus = null;
                    const deviceDetected = !!(processorMatch || deviceIdMatch);
                    if (deviceDetected) {
                        try {
                    ...
                    return {
                        success: deviceDetected, // Only success if device was actually detected
            ...
            if (!detectedMCUType) {
                processorMatch = errorOutput.match(/Cortex-(M\d+)/i);
                if (processorMatch) {
                    const processor = processorMatch[1];
            ...
            // Only return success if device was actually detected from error output
            const deviceDetectedFromError = !!(deviceIdMatch || processorMatch);
            if (deviceDetectedFromError) {
