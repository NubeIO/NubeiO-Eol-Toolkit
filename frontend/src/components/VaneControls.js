...
const VaneControls = ({ capabilities, isConnected, onUpdate }) => {
    ...
    return (
      <div className="space-y-2">
        ...
        {supportedArray.map((supported, idx) => (
          <div key={idx} className="flex items-center gap-2 text-xs bg-white/60 p-2 rounded border border-gray-200">
            <div className="w-14 font-medium">#{idx+1}</div>
            {!supported && <div className="text-gray-400 italic">N/A</div>}
            {supported && (
              <>
                <select
                ...
                {swingSupport && (
                  <button
                    ...
                    className="px-2 py-0.5 bg-indigo-500 hover:bg-indigo-600 text-white rounded"
                  >Swing</button>
                )}
                {swingSupport && (
                  <button
                    ...
                    className="px-2 py-0.5 bg-gray-400 hover:bg-gray-500 text-white rounded"
                  >Stop</button>
                )}
              </>
            )}
  ...
  return (
    <div className="bg-white/70 backdrop-blur rounded-lg p-4 shadow relative">
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-sm font-semibold text-gray-800">Vane Controls</h3>
        {busy && <span className="text-xs text-gray-500 animate-pulse">Updating...</span>}
      </div>
